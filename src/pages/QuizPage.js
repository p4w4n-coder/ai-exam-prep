...
import "./QuizPage.css";
import { loadQuizProgress, saveQuizProgress, clearQuizProgress } from "../utils/progressStorage";
...
export default function QuizPage({ quiz, onBack }) {
  ...
  // Restore in-progress attempt on mount, if one was saved within the last week
  useEffect(() => {
    const saved = loadQuizProgress(quiz.id);
    if (saved && Array.isArray(saved.questions) && saved.questions.length > 0) {
      setQuestions(saved.questions);
      setSectionTitle(saved.sectionTitle || "");
      setSectionIdx(saved.sectionIdx ?? null);
  ...
  // Persist progress continuously while a question is active
  useEffect(() => {
    if (view !== "quiz") return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, current, selected, revealed, answers, elapsed]);
  ...
  function startSection(idxOrAll) {
    const saved = loadQuizProgress(quiz.id);
    if (saved && Array.isArray(saved.questions) && saved.questions.length > 0 && saved.sectionIdx === idxOrAll) {
      setQuestions(saved.questions);
      setSectionTitle(saved.sectionTitle || "");
      setSectionIdx(saved.sectionIdx);
      setCurrent(saved.current ?? 0);
      setSelected(saved.selected ?? null);
      setRevealed(saved.revealed ?? false);
      setAnswers(Array.isArray(saved.answers) ? saved.answers : []);
  ...
  return (
    <div className="quiz-page">
      ...
      <div className="quiz-topbar">
        <button className="back-link" onClick={handleBackToSection}>
      ...
      {justResumed && (
        <div className="resume-banner">
          <span>
            Resuming where you left off — Question {current + 1} of {questions.length}
            {answers.length > 0 && ` · ${answers.length} answered so far`}
          </span>
          <button
            type="button"
