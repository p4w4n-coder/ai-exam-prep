...
const certifications = [
  ...
  },
  {
    provider: "Google",
    name: "GenAI Leader",
    description: "Practice resources for Google's GenAI Leader certification.",
...
export default function HomePage({ quizzes = [], onStart, onStudy, onAbout }) {
  ...
  const openStudy = (studyKey) => {
    if (onStudy) onStudy(studyKey);
  };
    ...
    <div className="aier-home" ref={revealRef}>
      <header className="aier-header">
        <div className="aier-container aier-nav">
          ...
          <nav className="aier-nav-links" aria-label="Main navigation">
            ...
            <button type="button" className="aier-nav-cta" onClick={scrollToCertifications}>
              Start Practicing →
            </button>
          </nav>
        </div>
          ...
          <div className="aier-container">
            ...
            <div className="aier-paths">
              {certifications.map((cert) => (
                <div className="aier-path" key={cert.provider}>
                  <span className="aier-tag">{cert.provider}</span>
                  <h3>{cert.name}</h3>
                  <p>{cert.description}</p>
                  <div className="aier-card-actions">
                    <button type="button" className="aier-card-practice" onClick={() => onStart(cert.firstSetId)}>
                      Practice
                    </button>
                    {cert.studyKey && (
                      <button type="button" className="aier-card-notes" onClick={() => openStudy(cert.studyKey)}>
                        Study Notes
                      </button>
                    )}
                  </div>
        ...
        <section id="resources" className="aier-section aier-reveal">
          <div className="aier-container">
            <div className="aier-section-head">
              ...
              <p>
                Structured lessons, real practice questions, and clear
                explanations — organized by certification.
              </p>
            </div>
      ...
      <footer className="aier-footer">
        <div className="aier-container aier-footer-inner">
          ...
          <button type="button" className="aier-footer-link" onClick={openAbout}>
            About
          </button>
        </div>
      </footer>
