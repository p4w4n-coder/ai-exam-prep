...
const EXAM_INFO = {
  ...
  "GAIL": {
    ...
    badge: "Google",
    badgeColor: "#4285F4",
    desc: "90 min · 4 domains · GenAI fundamentals + GCP offerings",
    icon: "🔷",
  },
};
...
export default function HomePage({ quizzes, onStart, onStudy, onAbout }) {
  ...
  const aifc01 = quizzes.filter(q => q.exam === "AIF-C01");
  const gail = quizzes.filter(q => q.exam === "GAIL");
  const totalQ = quizzes.reduce((a, q) => a + q.total, 0);
  const totalSets = quizzes.length;
  const totalExams = 4;
  ...
  return (
    <div className="home">
      ...
      <section className="hero">
        <div className="hero-inner">
          ...
          <div className="hero-stats">
            ...
            <div className="stat-div" />
            <div className="stat">
              <span className="stat-num">{totalExams}</span>
      ...
      {/* AIF-C01 section */}
      <section className="exam-section" id="aifc01">
        <div className="container">
          <div className="section-header">
            <div className="section-badge" style={{ background: "#FF9900" }}>AWS</div>
            <h2 className="section-title">AWS AIF-C01 — AI Practitioner</h2>
            <p className="section-desc">
              90 minutes · 65 questions · 700/1000 to pass · Covers GenAI fundamentals, foundation models,
              SageMaker services, Bedrock, responsible AI, and security. Service name recall is critical.
            </p>
          </div>
          <div className="cards-grid">
            {aifc01.map(q => <QuizCard key={q.id} quiz={q} onStart={onStart} />)}
          </div>
        </div>
      </section>
      ...
      {/* GAIL section */}
      <section className="exam-section" id="gail">
        <div className="container">
          <div className="section-header">
            <div className="section-badge" style={{ background: "#4285F4" }}>Google</div>
            <h2 className="section-title">Google Generative AI Leader</h2>
            <p className="section-desc">
              4 domains · GenAI fundamentals, Google Cloud GenAI offerings, techniques to improve output,
              and business strategies. Covers Vertex AI, Gemini family, RAG, and responsible AI.
            </p>
          </div>
          <div className="cards-grid">
            {gail.map(q => <QuizCard key={q.id} quiz={q} onStart={onStart} />)}
          </div>
        </div>
      </section>
